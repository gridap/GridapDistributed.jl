...
function Algebra.nz_allocation(a::DistributedCounterCOO)
  allocs = map(nz_allocation,a.counters) # Array{AllocationCOO}
...
function Algebra.create_from_nz(a::PSparseMatrix)
  # For LocallyAssembled the underlying Exchanger should
  # not have ghost layer making assemble! do nothing (TODO check)
  assemble!(a) |> wait
  a
end

function Algebra.create_from_nz(a::DistributedAllocationCOO{<:LocallyAssembled})
  f(x) = nothing
  A, = _fa_create_from_nz_with_callback(f,a)
  return A
end

function Algebra.create_from_nz(a::ArrayBlock{<:DistributedAllocationCOO{<:LocallyAssembled}})
  f(x) = nothing
  A, = _fa_create_from_nz_with_callback(f,a)
  return A
end
...
function Algebra.create_from_nz(a::DistributedAllocationCOO{<:Assembled})
  f(x) = nothing
  A, = _sa_create_from_nz_with_callback(f,f,a,nothing)
  return A
end

function Algebra.create_from_nz(a::ArrayBlock{<:DistributedAllocationCOO{<:Assembled}})
  f(x) = nothing
  A, = _sa_create_from_nz_with_callback(f,f,a,nothing)
  return A
end
...
function Algebra.create_from_nz(a::PVectorAllocationTrackOnlyValues{<:LocallyAssembled})
  rows = _setup_prange_without_ghosts(a.test_gids)
  _rhs_callback(a,rows)
end

function Algebra.create_from_nz(a::PVectorAllocationTrackOnlyValues{<:Assembled})
...
function Algebra.create_from_nz(
  a::DistributedAllocationCOO{<:LocallyAssembled},
  c_fespace::PVectorAllocationTrackOnlyValues{<:LocallyAssembled}
)
...
function Algebra.create_from_nz(
  a::DistributedAllocationCOO{<:Assembled},
...
function local_views(a::PVectorAllocationTrackTouchedAndValues,rows)
  @check PArrays.matching_local_indices(rows, a.test_gids)
  a.allocations
end
...
function Arrays.nz_allocation(
  a::DistributedCounterCOO{<:Assembled},
...
function _setup_matrix(
  a,I,J,V,rows::PRange,cols::PRange
)
  _rows = PRange(map(remove_ghost,partition(rows)))
  asys = change_axes(a,(_rows,cols))
  values = map(create_from_nz,local_views(asys))
  PSparseMatrix(values,partition(_rows),partition(cols),true)
end

function _setup_matrix(a,I,J,V,rows::Vector{<:PRange},cols::Vector{<:PRange})
  block_ids  = CartesianIndices(I)
  block_mats = map(block_ids) do id
    i = id[1]; j = id[2];
    _setup_matrix(a[i,j],I[i,j],J[i,j],V[i,j],rows[i],cols[j])
  end
...
# LocallyAssembled() parallel assembly strategy 
...
function _setup_prange_with_ghosts(dofs_gids_prange::PRange,gids)
  ...
  gids_ghost_to_global, gids_ghost_to_owner = map(
    find_gid_and_owner,gids_ghost_lids_to_dofs_ghost_lids,dofs_gids_partition
  ) |> tuple_of_arrays
