...
function FESpaces.collect_cell_matrix(
  trial::DistributedFESpace, test::DistributedFESpace, matdata::DistributedDomainContribution
)
  map(collect_cell_matrix,local_views(trial),local_views(test),local_views(matdata))
end

function FESpaces.collect_cell_vector(
  test::DistributedFESpace, vecdata::DistributedDomainContribution
)
  map(collect_cell_vector,local_views(test),local_views(vecdata))
end

function FESpaces.collect_cell_matrix_and_vector(
  trial::DistributedFESpace, test::DistributedFESpace,
  matdata::DistributedDomainContribution, vecdata::DistributedDomainContribution
)
  map(collect_cell_matrix_and_vector,
    local_views(trial), local_views(test),
    local_views(matdata), local_views(vecdata)
  )
end

function FESpaces.collect_cell_matrix_and_vector(
  trial::DistributedFESpace, test::DistributedFESpace,
  matdata::DistributedDomainContribution, vecdata::DistributedDomainContribution, uhd
)
  map(collect_cell_matrix_and_vector,
    local_views(trial), local_views(test),
    local_views(matdata), local_views(vecdata), local_views(uhd)
  )
end
...
function FESpaces.collect_cell_matrix_and_vector(
  trial::DistributedFESpace, test::DistributedFESpace,
  matdata::DistributedDomainContribution, l::Number
)
  map(local_views(trial),local_views(test),local_views(matdata)) do u,v,m
...
function FESpaces.collect_cell_matrix_and_vector(
  trial::DistributedFESpace, test::DistributedFESpace,
  matdata::DistributedDomainContribution, l::Number, uhd
)
  map(local_views(trial),local_views(test),local_views(matdata),local_views(uhd)) do u,v,m,f
