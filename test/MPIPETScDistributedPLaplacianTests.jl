...
function NLSolversBase.x_of_nans(x::PETSc.Vec{Float64}, Tf=eltype(x))
    similar(x)
end

function Base.copyto!(dest::PETSc.Vec{Float64}, src::PETSc.Vec{Float64})
   copy!(dest,src)
end

function LinearAlgebra.mul!(Y::PETSc.Vec{Float64},A::PETSc.Mat{Float64},B::PETSc.Vec{Float64})
   PETSc.C.chk(PETSc.C.MatMult(A.p,B.p,Y.p))
end

function LinearAlgebra.rmul!(Y::PETSc.Vec{Float64},A::Number)
  PETSc.scale!(Y,A)
end

struct PETScVecStyle <: Broadcast.BroadcastStyle end
Base.BroadcastStyle(::Type{PETSc.Vec{Float64}}) = PETScVecStyle()

"`v = find_petscvec(bc)` returns the first PETSc.Vec among the arguments."
...
find_petscvec(a::PETSc.Vec{Float64}, rest) = a
...
function Base.copyto!(v::PETSc.Vec{Float64}, bc::Base.Broadcast.Broadcasted{Base.Broadcast.DefaultArrayStyle{0}})
...
function Base.copyto!(v::PETSc.Vec{Float64}, bc::Base.Broadcast.Broadcasted{PETScVecStyle})
   if bc.f == identity
      @assert length(bc.args)==1
      @assert bc.args[1] isa PETSc.Vec{Float64}
      copy!(v,bc.args[1])
   elseif bc.f == +
      @assert bc.f == +
      @assert bc.args[1] isa PETSc.Vec{Float64}
      @assert bc.args[2] isa PETSc.Vec{Float64}
      PETSc.C.chk(PETSc.C.VecWAXPY(v.p, 1.0, bc.args[1].p, bc.args[2].p))
   else
...
function Base.maximum(::typeof(Base.abs), x::PETSc.Vec{Float64})
  norm(x,Inf)
end

function Base.any(::typeof(Base.isnan),x::PETSc.Vec{Float64})
   false
end

function NLsolve.check_isfinite(x::PETSc.Vec{Float64})
   true
end

function (dist::Distances.SqEuclidean)(a::PETSc.Vec{Float64}, b::PETSc.Vec{Float64})
  norm(a-b,2)^2
end 
